...
import com.git.hui.springai.app.repository.ConversationMessageRepository;
...
public class QaBoltService {
    ...
    public Flux<String> ask(String chatId, String question, Collection<MultipartFile> files) {
        ...
        // 打印文件上传日志
        log.info("接收到文件数量: {}, 文件列表: {}", files.size(),
                files.stream().map(f -> f.getOriginalFilename() + "(" + f.getContentType() + ", " + f.getSize() + "字节)").toList());
        ...
        return requestSpec.stream().content()
                ...
                .doOnComplete(() -> {
                    // 保存AI回答到数据库
                    saveMessage(chatId, "assistant", aiResponse.toString());
                });
    }
     ...
     * 保存上传文档记录到数据库
     */
    private void saveDocument(String chatId, MultipartFile file) {
        try {
